...
class SearchTrainViewController: UIViewController {
    ...
    var favoriteStations:(source:String,destination:String) = ("","")
    ...
    /// This methods loads the favorite source and destinations stations if available to the corresponding textfields.
    /// - Parameter sender: sender object (button)
    @IBAction func loadFavoriteStations(_ sender: Any) {
        if !favoriteStations.source.isEmpty && !favoriteStations.destination.isEmpty {
            sourceTxtField.text = favoriteStations.source
            destinationTextField.text = favoriteStations.destination
            transitPoints = favoriteStations
        } else {
            showAlert(title: "No Favorites", message: "Favorites stations not available", actionTitle: "Okay")
        }
    }
    
    /// If user presses this button the station codes available in SOURCE and DESTINATION textfields will be added as favorite.
    /// - Parameter sender: sender object (button)
    @IBAction func setAsFavoriteStations(_ sender: Any) {
        if !transitPoints.source.isEmpty && !transitPoints.destination.isEmpty {
            favoriteStations = transitPoints
        } else {
            showAlert(title: "Select Stations", message: "Please select some stations to make it favorite", actionTitle: "Okay")
        }
    }
}
